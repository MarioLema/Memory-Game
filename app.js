...
const VIEW = {
    ...
    addFace(id, faceId){
        document.getElementById(`avatar-${id}`).innerHTML = `<img src="images/face-${faceId}.png" alt=""></img>`;
    }
};
const MODIFIER = {
    ...
    randomAvatars(){
        for(let i = 0; i < DATA.avatarsArr.length/2; i++){
            DATA.avatarsArr.push(Math.floor(Math.random * 66) + 1);
        }
        let tempBoard = DATA.boardArr;
        while(tempBoard.length !== 0){
//need a shuffle algorithm to push an object with index and face index to avatars positions
        }
    }
};
